...
// --- FUNGSI PUSAT WAKTU (Tambahkan di bagian paling atas app.js Anda) ---
function getWIBDate() {
    const now = new Date();
    // Menambahkan 7 jam untuk zona waktu WIB
...
document.getElementById('formMaster').addEventListener('submit', function(e) {
    ...
    const kodeInput = document.getElementById('m_kode').value.trim().toUpperCase();
    const isAutoDate = document.getElementById('m_auto_date').checked;
    
    // Gunakan getWIBDate() agar selalu akurat WIB
    const tanggalValue = isAutoDate ? getWIBDate() : document.getElementById('m_tanggal').value;
    ...
    const isDuplicate = db.master.some((item, i) => {
        if (idx !== "" && i === parseInt(idx)) return false;
        return item.kode.toUpperCase() === kodeInput;
    });

    if (isDuplicate) {
        alert('Gagal: Kode barang "' + kodeInput + '" sudah terdaftar!');
        return;
    }
    ...
    const autoBox = document.getElementById('m_auto_date');
    if (autoBox) {
...
function editMaster(i) {
    const item = db.master[i];
    toggleForm('formMasterContainer', true);
    
    document.getElementById('m_index').value = i;
    document.getElementById('m_kode').value = item.kode;
    document.getElementById('m_nama').value = item.nama;
    document.getElementById('m_satuan').value = item.satuan;
    document.getElementById('m_tanggal').value = item.tanggal;
    
    // Cek apakah tanggal data tersebut adalah hari ini (WIB)
    const autoBox = document.getElementById('m_auto_date');
    autoBox.checked = (item.tanggal === getWIBDate());
    toggleDate(autoBox);
...
// --- 3. FUNGSI TOGGLE DATE ---
function toggleDate(checkbox) {
    const inputDate = document.getElementById('m_tanggal');
    if (!inputDate) return;

    if (checkbox.checked) {
        inputDate.value = getWIBDate(); // Selalu update ke WIB saat dicentang
...
// --- 4. EVENT UNTUK TOMBOL TAMBAH BARANG ---
// Pastikan tanggal update ke WIB hari ini saat tombol "Tambah" diklik
document.getElementById('btnToggleMaster').addEventListener('click', function() {
    const autoBox = document.getElementById('m_auto_date');
...
document.getElementById('formMasuk').addEventListener('submit', function(e) {
    ...
    const isAutoDate = document.getElementById('in_auto_date').checked;
    
    // Gunakan getWIBDate() untuk akurasi WIB
    const tanggalValue = isAutoDate ? getWIBDate() : document.getElementById('in_tanggal').value;
    ...
    // Reset Checkbox ke Auto & Refresh UI
    const autoBox = document.getElementById('in_auto_date');
    if (autoBox) {
        autoBox.checked = true;
        toggleDateMasuk(autoBox);
    }

    toggleForm('formMasukContainer', false);
    refreshAllTables();
    alert("Data berhasil disimpan!");
});
...
function editMasuk(i) {
    ...
    const barang = db.master.find(m => m.kode === item.kode);
    ...
    // Cek apakah tanggal item sama dengan hari ini (WIB) untuk checkbox
    const autoBox = document.getElementById('in_auto_date');
    autoBox.checked = (item.tanggal === getWIBDate());
    toggleDateMasuk(autoBox);

    const searchInput = document.getElementById('in_kode_search');
    if (searchInput) {
        searchInput.value = barang ? `${item.kode} - ${barang.nama}` : item.kode;
    }
...
// --- 3. FUNGSI UTILITY TANGGAL ---
function toggleDateMasuk(checkbox) {
    const inputDate = document.getElementById('in_tanggal');
    if (!inputDate) return;
    
    if (checkbox.checked) {
        // Selalu update ke tanggal WIB terbaru saat dicentang
        inputDate.value = getWIBDate();
        inputDate.readOnly = true;
        inputDate.classList.add('bg-slate-100');
    } else {
        inputDate.readOnly = false;
        inputDate.classList.remove('bg-slate-100');
    }
}

// --- 4. EVENT LISTENER BUKA FORM ---
// Pastikan tanggal update ke WIB hari ini saat tombol "Tambah Masuk" diklik
document.getElementById('btnToggleMasuk').addEventListener('click', function() {
    const autoBox = document.getElementById('in_auto_date');
    if (autoBox && autoBox.checked) {
        toggleDateMasuk(autoBox);
    }
...
document.getElementById('formKeluar').addEventListener('submit', function(e) {
    ...
    const isAutoDate = document.getElementById('out_auto_date').checked;
    
    // Menggunakan fungsi getWIBDate() untuk akurasi WIB
    const tanggalValue = isAutoDate ? getWIBDate() : document.getElementById('out_tanggal').value;
    ...
    // LOGIKA STOK SAMA PERSIS DENGAN KODE ASLI ANDA
    const currentIdx = idx === "" ? -1 : parseInt(idx);
    if(data.qty > hitungStokBagus(data.kode, currentIdx, 'keluar')) { 
    ...
    // Reset Checkbox ke mode Auto
    const autoBox = document.getElementById('out_auto_date');
    if (autoBox) {
        autoBox.checked = true;
        toggleDateKeluar(autoBox);
    }
...
function editKeluar(i) {
    ...
    // Sinkronisasi checkbox tanggal menggunakan getWIBDate()
    const autoBox = document.getElementById('out_auto_date');
    autoBox.checked = (item.tanggal === getWIBDate());
    toggleDateKeluar(autoBox);
    
    // Sinkronisasi input search
    const searchInput = document.getElementById('out_kode_search');
    if (searchInput) {
        searchInput.value = barang ? `${item.kode} - ${barang.nama}` : item.kode;
    }
    ...
    document.getElementById('out_qty').focus();
}

// --- 3. FUNGSI UTILITY TANGGAL ---
function toggleDateKeluar(checkbox) {
    const inputDate = document.getElementById('out_tanggal');
    if (!inputDate) return;
    
    if (checkbox.checked) {
        // Selalu update ke tanggal WIB hari ini
        inputDate.value = getWIBDate();
        inputDate.readOnly = true;
        inputDate.classList.add('bg-slate-100');
    } else {
...
// --- 4. EVENT LISTENER BUKA FORM ---
// Memastikan tanggal selalu update ke WIB hari ini saat tombol buka form diklik
document.getElementById('btnToggleKeluar').addEventListener('click', function() {
    const autoBox = document.getElementById('out_auto_date');
    if (autoBox && autoBox.checked) {
        toggleDateKeluar(autoBox);
    }
...
document.getElementById('formRusak').addEventListener('submit', function(e) {
    ...
    const isAutoDate = document.getElementById('bad_auto_date').checked;
    
    // Gunakan getWIBDate() untuk akurasi WIB
    const tanggalValue = isAutoDate ? getWIBDate() : document.getElementById('bad_tanggal').value;
    ...
    // LOGIKA STOK SAMA PERSIS DENGAN KODE ASLI ANDA
    const currentIdx = idx === "" ? -1 : parseInt(idx);
    if(data.qty > hitungStokBagus(data.kode, currentIdx, 'rusak')) { 
        alert('Stok tidak cukup!'); 
        return; 
    }
    ...
    // Reset Checkbox ke mode Auto (default) & Refresh UI
    const autoBox = document.getElementById('bad_auto_date');
    if (autoBox) {
        autoBox.checked = true;
        toggleDateRusak(autoBox);
    }
...
function editRusak(i) {
    ...
    // Cari detail nama barang
    const barang = db.master.find(m => m.kode === item.kode);

    // Isi data ke form
    document.getElementById('bad_index').value = i;
    document.getElementById('bad_tanggal').value = item.tanggal;
    document.getElementById('bad_kode').value = item.kode;
    document.getElementById('bad_qty').value = item.qty;
    document.getElementById('bad_ket').value = item.ket;
    ...
    // Sinkronisasi input search
    const searchInput = document.getElementById('bad_kode_search');
    if (searchInput) {
        searchInput.value = barang ? `${item.kode} - ${barang.nama}` : item.kode;
    }
    ...
    document.getElementById('bad_qty').focus();
}

// --- 3. FUNGSI UTILITY TANGGAL ---
function toggleDateRusak(checkbox) {
    const inputDate = document.getElementById('bad_tanggal');
    if (!inputDate) return;
...
// --- 4. EVENT LISTENER BUKA FORM ---
// Pastikan tanggal update ke WIB hari ini saat form dibuka
document.getElementById('btnToggleRusak').addEventListener('click', function() {
    const autoBox = document.getElementById('bad_auto_date');
    if (autoBox && autoBox.checked) {
        toggleDateRusak(autoBox);
    }
...
// ================= LOGIKA REPORT GENERATION (DENGAN SATUAN) =================
function generateReport(tipe = 'ALL') {
    ...
    // Helper untuk mengambil satuan dari db.master
    const getSatuan = (kode) => {
        const item = db.master.find(m => m.kode.trim() === kode.trim());
        return item ? (item.satuan || 'Pcs') : 'Pcs'; // Default ke 'Pcs' jika satuan kosong
    };

    let htmlReport = `
    <div style="text-align: center; border-bottom: 3px double #000; padding-bottom: 10px; margin-bottom: 20px;">
        <h2 style="margin: 0; font-size: 20px; text-transform: uppercase;">Aplication SyncroStock- ${tipe.toUpperCase()}</h2>
        <p style="margin: 5px 0 0 0; font-size: 11px;">PT. AKMAL JAYA SENTOSA - LOGISTIK & PERGUDANGAN</p>
        <p style="margin: 2px 0 0 0; font-size: 10px; font-style: italic;">Periode: ${formatTanggalIndo(start)} s/d ${formatTanggalIndo(end)}</p>
    </div>`;

    // 1. Barang Masuk
    if (tipe === 'ALL' || tipe === 'masuk') {
        htmlReport += `<h3 class="report-section-title">Log Barang Masuk</h3>
        <table class="report-table" border="1" cellpadding="6" style="width: 100%; border-collapse: collapse;">
            <thead><tr><th>No</th><th>Tanggal</th><th>Kode</th><th>Nama Barang</th><th>Stok Keluar</th><th>Stok Awal</th><th>Sisa Stok</th><th>Satuan</th><th>Keperluan</th></tr></thead>
            <tbody>${dataMasukFiltered.length === 0 ? '<tr><td colspan="9" align="center">Tidak ada transaksi</td></tr>' : 
            dataMasukFiltered.map((x, i) => {
                const stokAkhir = getStokBagus(x.kode);
                const stokAwal = stokAkhir - x.qty;
                return `<tr><td>${i+1}</td><td>${formatTanggalIndo(x.tanggal)}</td><td>${x.kode}</td><td>${db.master.find(m => m.kode === x.kode)?.nama || '-'}</td><td>${stokAwal}</td><td style="color: green; font-weight: bold;">+${x.qty}</td><td>${stokAkhir}</td><td>${getSatuan(x.kode)}</td><td>${x.ket}</td></tr>`;
            }).join('')}</tbody>
        </table>`;
    }
    ...
    if (tipe === 'ALL' || tipe === 'keluar') {
        htmlReport += `<h3 class="report-section-title">Log Barang Keluar</h3>
        <table class="report-table" border="1" cellpadding="6" style="width: 100%; border-collapse: collapse;">
            <thead><tr><th>No</th><th>Tanggal</th><th>Kode</th><th>Nama Barang</th><th>Stok Awal</th><th>Stok Keluar</th><th>Sisa Stok</th><th>Satuan</th><th>Pekerja</th></tr></thead>
            <tbody>${dataKeluarFiltered.length === 0 ? '<tr><td colspan="9" align="center">Tidak ada transaksi</td></tr>' : 
            dataKeluarFiltered.map((x, i) => {
                const stokAkhir = getStokBagus(x.kode);
                const stokAwal = stokAkhir + x.qty;
                return `<tr><td>${i+1}</td><td>${formatTanggalIndo(x.tanggal)}</td><td>${x.kode}</td><td>${db.master.find(m => m.kode === x.kode)?.nama || '-'}</td><td>${stokAwal}</td><td style="color: blue; font-weight: bold;">-${x.qty}</td><td>${stokAkhir}</td><td>${getSatuan(x.kode)}</td><td>${x.ket}</td></tr>`;
            }).join('')}</tbody>
        </table>`;
    }

    // 3. Barang Rusak
    if (tipe === 'ALL' || tipe === 'rusak') {
        htmlReport += `<h3 class="report-section-title">Log Barang Rusak</h3>
        <table class="report-table" border="1" cellpadding="6" style="width: 100%; border-collapse: collapse;">
            <thead><tr><th>No</th><th>Tanggal</th><th>Kode</th><th>Nama Barang</th><th>Qty Barang Bekas</th><th>Sisa Stok</th><th>Satuan</th><th>Kronologi</th></tr></thead>
            <tbody>${dataRusakFiltered.length === 0 ? '<tr><td colspan="8" align="center">Tidak ada data</td></tr>' : 
            dataRusakFiltered.map((x, i) => `<tr><td>${i+1}</td><td>${formatTanggalIndo(x.tanggal)}</td><td>${x.kode}</td><td>${db.master.find(m => m.kode === x.kode)?.nama || '-'}</td><td style="color: red; font-weight: bold;">${x.qty}</td><td>${getStokBagus(x.kode)}</td><td>${getSatuan(x.kode)}</td><td>${x.ket}</td></tr>`).join('')}</tbody>
        </table>`;
    }
...
// Fungsi Cetak (Parameter 'tipe' default ke ALL)
function cetakReport(tipe = 'ALL') {
    generateReport(tipe); 
    setTimeout(() => { window.print(); }, 300);
}

// Fungsi Download (Parameter 'tipe' ditambahkan untuk memilih menu)
function exportTable(format, tipe = 'ALL') {
    generateReport(tipe); // Pastikan tabel sesuai menu yang di-generate
    let reportContent = document.getElementById('previewReport').innerHTML;
    
    const style = `<style>table { border-collapse: collapse; width: 100%; } th, td { border: 1px solid black; padding: 5px; text-align: center; }</style>`;
    const fullHtml = `<html><head><meta charset="utf-8">${style}</head><body>${reportContent}</body></html>`;

    let blob = new Blob(['\ufeff', fullHtml], { type: format === 'word' ? 'application/msword' : 'application/vnd.ms-excel' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `Laporan_${tipe}_${new Date().getTime()}.${format === 'word' ? 'doc' : 'xls'}`;
    link.click();
}
...
function showSuggestions(prefix, keyword) {
    const suggestionsDiv = document.getElementById(`${prefix}_suggestions`);
    const hiddenInput = document.getElementById(`${prefix}_kode`);
    
    // Jika input dihapus, reset hidden input
    if (keyword.length === 0) {
        hiddenInput.value = '';
    }
    ...
    const matches = db.master.filter(m => 
        m.kode.toLowerCase().includes(keyword.toLowerCase()) || 
        m.nama.toLowerCase().includes(keyword.toLowerCase())
    );

    if (matches.length > 0) {
        suggestionsDiv.innerHTML = matches.map(m => {
            // Kita gunakan escape agar karakter khusus pada nama barang tidak merusak fungsi
            const displayName = `${m.kode} - ${m.nama}`;
            const escapedNama = m.nama.replace(/'/g, "\\'"); 
            
            return `
            <div class="px-4 py-2 cursor-pointer hover:bg-blue-50 text-sm border-b border-slate-100" 
                 onclick="selectItem('${prefix}', '${m.kode}', '${escapedNama}')">
                ${displayName}
            </div>`;
        }).join('');
        suggestionsDiv.classList.remove('hidden');
    } else {
        suggestionsDiv.innerHTML = '<div class="px-4 py-2 text-slate-400 text-sm italic">Tidak ditemukan</div>';
        suggestionsDiv.classList.remove('hidden');
    }
}

// Fungsi saat user mengklik salah satu saran
function selectItem(prefix, kode, nama) {
    document.getElementById(`${prefix}_kode_search`).value = `${kode} - ${nama}`;
    document.getElementById(`${prefix}_kode`).value = kode;
    document.getElementById(`${prefix}_suggestions`).classList.add('hidden');
}

// Tambahan: Pastikan menutup saran jika user klik di luar area
document.addEventListener('click', (e) => {
    // Jika target klik BUKAN merupakan elemen input search atau suggestions
    if (!e.target.id.includes('_kode_search') && !e.target.closest('[id$="_suggestions"]')) {
        document.querySelectorAll('[id$="_suggestions"]').forEach(el => el.classList.add('hidden'));
    }
});