...
document.getElementById('formMaster').addEventListener('submit', function(e) {
    ...
    // Tentukan tanggal (Otomatis hari ini atau Manual)
    const tanggalValue = isAutoDate 
        ? new Date().toISOString().split('T')[0] 
        : document.getElementById('m_tanggal').value;
    ...
    // --- LOGIKA VALIDASI DUPLIKASI ---
    const isDuplicate = db.master.some((item, i) => {
        // Abaikan pengecekan jika index yang sama (saat proses Update)
    ...
    if (idx === "") { 
        ...
        // Update: Update referensi transaksi jika kode berubah
    ...
    // Kembalikan checkbox ke mode Auto (default)
...
function editMaster(i) {
    ...
    // Cek apakah tanggal adalah hari ini untuk menentukan checkbox
    const today = new Date().toISOString().split('T')[0];
    const autoBox = document.getElementById('m_auto_date');
    autoBox.checked = (item.tanggal === today);
    toggleDate(autoBox);
...
document.getElementById('formMasuk').addEventListener('submit', function(e) {
    ...
    // Ambil index dan checkbox status
...
function editMasuk(i) {
    ...
    // Cek apakah tanggal hari ini untuk checkbox
    const today = new Date().toISOString().split('T')[0];
    const autoBox = document.getElementById('in_auto_date');
    autoBox.checked = (item.tanggal === today);
    toggleDateMasuk(autoBox);

    // Isi teks search agar user melihat nama barang
...
document.getElementById('formKeluar').addEventListener('submit', function(e) {
    ...
    const tanggalValue = isAutoDate 
        ? new Date().toISOString().split('T')[0] 
        : document.getElementById('out_tanggal').value;
...
document.getElementById('formRusak').addEventListener('submit', function(e) {
    ...
    const tanggalValue = isAutoDate 
        ? new Date().toISOString().split('T')[0] 
        : document.getElementById('bad_tanggal').value;
